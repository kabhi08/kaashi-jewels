...
const Rings = () => {
  ...
  // 👉 Fetch ring products
  useEffect(() => {
    axios.get('http://localhost:5000/api/products/category/rings')
      .then(res => {
        setRings(res.data);
  ...
  const handleFilterChange = (value) => {
    ...
    switch (value) {
      case 'under1000':
        setFiltered(rings.filter(p => p.price < 1000));
        break;
      case '1000to5000':
        setFiltered(rings.filter(p => p.price >= 1000 && p.price <= 5000));
        break;
      case 'above5000':
        setFiltered(rings.filter(p => p.price > 5000));
        break;
      default:
        setFiltered(rings);
    }
    ...
    <div className={`product-grid ${isDark ? 'dark-theme' : 'light-theme'}`}>
      <div className="necklace-header">
        ...
        <div className="text-left center-text">
          ...
          <p>Explore our exquisite collection of rings that add elegance to any outfit.</p>
        </div>
